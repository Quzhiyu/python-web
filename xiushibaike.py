...
class QSBK:
	...
	def getPageItems(self,pageIndex):
		...
		item = re.findall('<div.*?class="author.*?>.*?<a.*?</a>.*?<a.*?>(.*?)</a>.*?<div.*?class'+
			'="content".*?title="(.*?)">(.*?)">(.*?)</div>(.*?)<div class="stats.*?class="number">(.*?)</i>',pageCode)
		#用来存储每页的段子们
		pageStories = []
		#遍历正则表达式匹配的信息
		for item in items:
	...
	def getOneStory(self,pageStories,page):
		...
		for story in pageStories:
			...
			if  input == "Q":
				self.enable = False
				return
			print u"第%d页\t发布人:%s\t发布时间:%s\n%s\n赞：%s" %(page,story[0],story[1],story[2],story[3])

	#开始方法
	def start(self):
		print u"正在读区糗事百科，按回车查看新段子，Q退出"
		#使变量为True，程序可以正常运行
		self.enable = True
		#先加载一页内容
		self.loadPage()
